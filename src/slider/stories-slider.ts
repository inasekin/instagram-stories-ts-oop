...
export default class StoriesSlider {
  ...
  constructor(slider_init: SliderSettings) {
    ...
    // Add style block
    this.settings.sliders_container.insertAdjacentHTML(
      'afterend',
      `<div id='sliders-styles'><style></style></div>`
    );
    this.settings.sliders_style = document.getElementById('sliders-styles')
    ...
    this.settings.sliders_bg_modal_close.addEventListener('click', () =>
  ...
  _create_global_settings(
    ...
    return {
      sliders_container: document.getElementsByClassName(
        settings.sliders_container_class.replace('.', '')
      )[0],
      sliders_bg_modal: document.getElementsByClassName(
        settings.sliders_bg_modal_class.replace('.', '')
      )[0],
      sliders_bg_modal_close: document.getElementsByClassName(
        settings.sliders_bg_modal_close_class.replace('.', '')
      )[0],
      ...settings,
    };
  }
  ...
  _create_new_slider(slider_settings: SlideItemSettings): SlideItem {
    let slider_container = document.getElementById(
      slider_settings.id.toString()
    );
    let trigger_container = document.querySelector(
      `[data-slider-history-id-on-click="${slider_settings.id}"]`
    );
    let slides_container = document
      .getElementById(slider_settings.id.toString())
      .querySelector('[data-slide-items-container]');

    let header_container = document
      .getElementById(slider_settings.id.toString())
      .querySelector(this.settings.sliders_header_class);
    ...
    return <SlideItem>{
      ...
      is_last_slide:
        slider_settings.active_slide_number >=
        slides_container.childElementCount,
      ...
      slides: {
        ...
        auto_slide: [
          ...Array.from(slides_container.querySelectorAll('.item')),
        ].map((_) => {
          return {
            active: true,
            timeout: undefined,
  ...
  _create_nav_slider(slider_id: string): void {
    ...
    header.innerHTML = `${nav_open} 
                            ...
                            ${nav_close}`;
    document
      .getElementById(slider_id)
      .insertAdjacentHTML('afterbegin', header.outerHTML);
  }

  _create_menu_slider(slider_item: SlideItem): void {
    let slider_img = slider_item.trigger_container
      .querySelectorAll('[data-slide-img]')[0]
      .cloneNode(true) as HTMLElement;
    let slider_name = slider_item.trigger_container
      .querySelectorAll('[data-slide-title]')[0]
      .cloneNode(true) as HTMLElement;
    ...
    slider_item.header.menu.container.insertAdjacentHTML(
      'afterbegin',
      slider_menu
    );
  }
  ...
  _create_thumb_slider(slider_item: SlideItem): void {
    ...
    slider_item.header.thumb.items =
      slider_item.header.thumb.container.querySelectorAll('*');
  }
  ...
  _add_slider_handler_click(slider_item: SlideItem): void {
    ...
    slider_item.trigger_container.addEventListener('click', () =>
      slider_item.show()
    );
    slider_item.header.next.container.addEventListener('click', () =>
      slider_item.next_slide()
    );
    slider_item.header.prev.container.addEventListener('click', () =>
      slider_item.prev_slide()
    );
  }

  _set_to_style_animate_duration(
    thumb_slide_id: string,
    animate_duration: string
  ) {
  ...
  _processing_slide_items(slider_item: SlideItem): void {
    slider_item.slides.items.forEach((slide, slide_index) => {
      ...
      let slide_thumb = slider_item.header.thumb.items[
        slide_index
      ] as HTMLElement;
      ...
      let result: ISliderItemTypes = {
        html: () => {
          ...
          this._set_to_style_animate_duration(
            slide_thumb.dataset.thumbSlideId,
            slide_thumb.dataset.timeDuration
          );
        ...
        video: () => {
          let video_elem = _slide.getElementsByTagName(
            'video'
          )[0] as HTMLVideoElement;
          video_elem.onloadedmetadata = () => {
            slider_item.slides.auto_slide[slide_index].time =
              video_elem.duration * 1000;
            slide_thumb.dataset.timeDuration = `${
              video_elem.duration * 1000
            }ms`;
            this._set_to_style_animate_duration(
        ...
        default: () => {
          console.error(
            `Not found this type slide | get type: ${_slide.dataset.slideItemType}`,
            slide
          );
          slider_item.slides.items_type.push(
            <SliderItemTypes.Default>'default'
          );
        },
      };
  ...
  _hide_all_slider(): void {
    for (const slider_id in this.sliders) {
      ...
      if (
        this.sliders[slider_id].slides.items_type.includes(
          <SliderItemTypes.Video>'video'
        )
      ) {
        this.sliders[slider_id].slides.items_type.map((item, index) => {
          if (item != <SliderItemTypes.Video>'video') return;

          let active_slide = this.sliders[slider_id].slides.items[
            index
          ] as HTMLElement;
          let slide_video = active_slide.getElementsByTagName(
            'video'
          )[0] as HTMLVideoElement;
          slide_video.pause();
  ...
  _activate_slide(
    slider_item: SlideItem,
    old_slide_id: number,
    new_slide_id: number
  ): void {
    let old_slider_items = slider_item.slides.items[
      old_slide_id
    ] as HTMLElement;
    old_slider_items.classList.remove('active');

    let old_slider_thumb = slider_item.header.thumb.items[
      old_slide_id
    ] as HTMLElement;
    ...
    let new_slider_items = slider_item.slides.items[
      new_slide_id
    ] as HTMLElement;
    new_slider_items.classList.add('active');

    let new_slider_thumb = slider_item.header.thumb.items[
      new_slide_id
    ] as HTMLElement;
    new_slider_thumb.classList.add('active');

    slider_item.active_slide_id = new_slide_id;
    if (slider_item.slides.auto_slide[slider_item.active_slide_id].active)
      slider_item.activate_auto_slide();

    slider_item.is_last_slide = new_slide_id + 1 >= slider_item.slides.count;

    slider_item.is_first_slide = new_slide_id - 1 < 0;
  ...
  _activate_auto_slide(slider_id: string): void {
    ...
    slider_item.slides.auto_slide[slider_item.active_slide_id].timeout =
      setTimeout(
        slider_item.next_slide,
        slider_item.slides.auto_slide[slider_item.active_slide_id].time
      );
  }

  _get_next_slider_id(slider_id: string): string {
    let next_slider_id =
      this.sliders_keys[Object.keys(this.sliders).indexOf(slider_id) + 1];
  ...
  _get_prev_slider_id(slider_id: string): string {
    ...
    if (!prev_slider_id && this.settings.hasOwnProperty('loop')) {
      if (this.settings.loop)
        prev_slider_id = Object.keys(this.sliders)[this.count_sliders - 1];
    }
  ...
  _set_status_is_viewed_slider(slider_item: SlideItem): void {
    ...
    let border_img = slider_item.trigger_container.querySelector(
      '[data-slide-border]'
    );
    border_img.getElementsByTagName('svg')[0].style.stroke =
      this.settings.sliders_border_color_viewed;
  }
  ...
  _start_active_slide(slider_item: SlideItem) {
    slider_item.slides.items_type.forEach((slide_type, slide_index) => {
      ...
      let active_slide = slider_item.slides.items[
        slider_item.active_slide_id
      ] as HTMLElement;
      let slide_video = active_slide.getElementsByTagName(
        'video'
      )[0] as HTMLVideoElement;
      ...
      if (slide_index !== slider_item.active_slide_id)
        slide_video.currentTime = 0;
    });
  }
  ...
  next_slide(slider_id: string): void {
    ...
    if (
      (this.next_slider_id == this.active_slider_id || !this.next_slider_id) &&
      slider.is_last_slide &&
      !this.settings.loop
    ) {
      this._hide_all_slider();
    ...
    if (slider.is_last_slide) {
      ...
      let last_slide = slider.header.thumb.items[
        slider.header.thumb.items.length - 1
      ] as HTMLElement;
      last_slide.classList.remove('active');
      last_slide.classList.add('viewed');

      if (typeof this.next_slider_id === 'string')
        this.sliders[this.next_slider_id].show();
      return;
    }

    slider.next_slide_id =
      slider.active_slide_id + 1 < slider.slides.items.length
        ? slider.active_slide_id + 1
        : 0;
    slider.prev_slide_id =
      slider.active_slide_id - 1 >= 0
        ? slider.active_slide_id - 1
        : slider.slides.items.length - 1;
  ...
  prev_slide(slider_id: string) {
    ...
    if (slider.is_first_slide) {
      ...
      if (typeof this.prev_slider_id === 'string')
        this.sliders[this.prev_slider_id].show();
  ...
  show(slider_id: string): void {
    ...
    this._activate_slide(
      this.sliders[slider_id],
      0,
      this.sliders[slider_id].active_slide_id
    );
    ...
    if (next_slider_id) {
      this.sliders[next_slider_id].slider_container.classList.add(
        'slider-next'
      );
    ...
    if (prev_slider_id) {
      this.sliders[prev_slider_id].slider_container.classList.add(
        'slider-prev'
      );
  ...
  hide(slider_id: string): void {
    ...
    // Reset active thumb
    slider.header.thumb.items.forEach((item) =>
